...
test('keeps the translation edition identity, settings, and user data isolated', () => {
  assert.equal((viteConfig.match(/__APP_EDITION__.*APP_EDITION.*creative/g) || []).length, 2)
  ...
  assert.match(defaultLayout, /v-if="!translationEdition" class="window-no-drag"/)
  assert.match(rendererI18n, /__APP_EDITION__ === 'translation'[\s\S]*updateLocale\('zh-CN'\)/)
