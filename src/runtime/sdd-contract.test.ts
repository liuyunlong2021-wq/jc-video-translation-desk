...
test('installs the local subtitle and source-separation engines from the app', () => {
  ...
  assert.match(videoTranslationOcr, /reportProgress\('正在获取画面字幕'\)/)
  assert.doesNotMatch(videoTranslationOcr, /视频字幕 OCR 正在读取画面字幕/)
  ...
  assert.match(funAsrRuntime, /active_height \* 0\.62/)
  assert.match(funAsrRuntime, /active_height \* 0\.96/)
  assert.doesNotMatch(funAsrRuntime, /active_height \* 0\.35/)
...
test('frame calibration uses retryable 20-cue project task batches instead of a hard cue limit', () => {
  ...
  assert.match(homeUi, /autoIdentifyVisualPeople\(state\)/)
  assert.doesNotMatch(homeUi, /画面识别人物会按每批 20 条/)
