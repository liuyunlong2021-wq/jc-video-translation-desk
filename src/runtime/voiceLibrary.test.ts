...
test('lists only confirmed IndexTTS2 packs and binds a confirmed drama character with backlinks', async () => {
  ...
  fs.writeFileSync(path.join(packDir, 'manifest.json'), JSON.stringify({
    status: 'confirmed',
    model: { id: 'indextts-2' },
    source: { referenceRelativePath: `packs/${profile.voiceProfileId}/zh/indextts-2/source-reference.wav` },
    emotions: { neutral: { audio: 'neutral.wav', status: 'generated', confirmed: true } },
  }))
  assert.deepEqual(
    (await voices.listVoiceProfiles({ indexTtsReady: true })).map((item: any) => item.voiceProfileId),
    [profile.voiceProfileId],
  )

  const projectId = 'drama-project'
  const wiki = path.join(userData, 'media-runs', projectId, 'wiki')
  ...
  fs.writeFileSync(path.join(wiki, '项目', '项目总监.md'), '# 项目总监\n\n- [[资产/角色/character-1|角色一]]\n')
  fs.writeFileSync(path.join(wiki, '资产', '角色', 'character-1.md'), '---\nentityId: character-1\n---\n\n# 角色一\n')
