...
字幕工作台允许人工输入开始时间、结束时间和漏识别台词，新增后立即按时间顺序写入字幕表并重新进入翻译确认。字幕生成后即可进入成片工作台，也可以随时返回字幕工作台或配音工作台。成片工作台复用同一时间轴与双语字幕，但固定隐藏“说话角色”列和左侧角色库，只负责音频、字幕烧录和成片导出；尚未生成整集目标语言配音时，只禁用依赖该音轨的混音和烧录，不隐藏成片工作台。

配音工作台角色列表使用该角色第一条已绑定台词对应的原片画面作为低清识别头像。字幕工作台左侧允许删除误建角色；删除动作同步移除共享角色状态和该角色的翻译 Wiki 文件，并解除当前字幕绑定，但不删除共享声音库文件。角色参考音上传必须写入 `episodes/<episodeId>/video-translate/seed-audio/uploads/`，再由翻译沙箱校验和登记。角色音色提示词只读取当前角色的身份资料和当前角色已确认的全部目标语言台词，不混入其他角色台词。
...
全局配音：生成全局配音提示词 / 生成或重新生成全局配音 / 进入成片工作台
```
...
27. 顶部始终能看到字幕工作台、配音工作台和成片工作台；成片工作台不显示角色列，只显示时间轴、预览、双语字幕和目标语言配音，未生成配音时只阻塞依赖音轨的后续动作。
