...
> 2026-08-10 产品边界更新：本仓库只保留“视频翻译工作台”单一产品，不再保留“点一点”产品入口和构建模式。本文后续出现的“内容创作 / 视频翻译双入口”均为历史设计，不再执行；底层共用能力和旧项目读取兼容继续保留。
> 定位：独立的视频翻译 Electron 桌面应用
...
- 已按“字幕工作台 -> 配音工作台 -> 成片工作台”拆分职责；字幕工作台负责扒片和双语字幕确认，视频翻译配音工作台复用 `角色配音 / 分组克隆`，创作主应用继续保留 Seed `全局配音`，成片工作台不再显示角色列。
...
视频翻译工作台把用户已有视频送入现有的大模型、FFmpeg、角色声音库、Seed Audio 和 Spleeter 能力，输出目标语言配音与字幕成片。
...
以下“双入口”内容属于历史设计，已由单一视频翻译产品决策取代：
...
- 第二阶段复用现有 `角色配音 / 分组克隆` 工作台，完成参考音绑定、连续对白提示词和对齐后的目标语言人声；创作模式才使用 `全局配音`；
...
角色声音特征属于 `translationRoleId`，不属于某个参考音文件。角色配音工作台根据整集剧情上下文、当前角色已确认的全部目标语言台词和角色身份生成一份当前角色声音提示词；英文目标默认自然美式英语口音。该提示词可以手工修改或 AI 集中修改，并作为分组克隆和创作模式全局配音共同使用的角色声音事实源。生成参考音、上传参考音或选择现成参考音只改变当前音频绑定，不覆盖角色声音特征。
...
视频翻译模式的“角色配音”与“分组克隆”按钮在 Mac 和 Windows 上一致显示。“进入成片工作台”始终可见，只有在当前采用的分组克隆版本不完整时置灰并说明原因。上传、单角色生成、选择已有参考音和确认角色声音保留在角色详情中。创作模式的全局配音页继续提供生成提示词、生成配音、试听和版本选择，但不属于视频翻译模式入口。
...
目标语言使用明确的语言代码或 BCP-47 标签。中文、英文及东南亚字幕语种可在字幕工作台翻译；越南语、泰语、印尼语、马来语（马来西亚）的 Seed Audio 配音效果尚未完成真实验收，因此暂不把它们的配音质量写成已确认结论。
