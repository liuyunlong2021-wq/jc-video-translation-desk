...
- 已增加独立的视频翻译入口、三栏 UI、六列字幕工作台和右栏 10 个固定动作；扒片是单一入口。
...
- 已通过 `pnpm test`（180/180）、`vue-tsc --noEmit`、`git diff --check`、APP 签名和 DMG 校验。
...
- 中栏是视频字幕工作台；
- 右栏只显示执行按钮和已生成的豆包语音稿；不显示临时角色声音选择区；
...
选中字幕行只驱动中栏预览和编辑，不在右栏显示声音选择控件。正式声音设计与绑定由后续配音字幕工作台负责。
...
工作台中的修改保存在翻译运行态；点击“确认角色与字幕”时，把确认后的完整快照追加到 `过程记录.md`，并生成下面的正式产物。人工确认不改写四步扒片记录。
...
14. 右栏不显示临时角色声音选择区；正式声音绑定由后续配音字幕工作台处理。
...
27. 现有配音字幕工作台与后端回归测试保持通过。
28. `pnpm test`、`pnpm exec vue-tsc --noEmit`、`git diff --check` 通过。
