...
> 多集扩展：项目/本集所有权、项目总监资产复用绑定和所有 `episodeId` 路径以 `docs/多集项目与共享资产架构SDD.md` 为准；本文中的 `episode-001` 仅表示当前集示例，不得继续硬编码。
> 相关 Skill：`skills/jc-film-style/SKILL.md`、`skills/jc-script-storyboard/SKILL.md`、`skills/jc-voice-design/SKILL.md`、`skills/jc-doubao-seed-audio/SKILL.md`
> 替代关系：本版完整替代 2026-08-02 的“统一后置配音、禁用 Whisper”方案；旧结论不再参与实现。

## 1. 一句话目标

文稿确认后先由项目总监判断内容类型并建立角色、场景、道具和导演事实，再由用户人工选择声音制作路线。Seed 路线在声音资产确定后，先生成可编辑豆包语音稿，再生成完整声音轨并用 Faster-Whisper 得到真实声音时间轴，画面导演随后按声音节奏设计分镜；逐句后配路线保持先生成视频、再后配。两条路线的视频素材都在生成后由 Faster-Whisper 和 Gemini 得到真实剪辑点，最后进入同一个配音字幕工作台完成成片。

## 2. 已确认决策

1. 项目总监必须在声音生成、资产和分镜之前完成，并负责判定内容类型；声音制作路线由用户人工选择，用户可以在项目总监页修改内容类型并重新确认，不被 AI 首次判断锁死。
2. 内容类型只允许 `narration-promo` 和 `drama`；AI 给出初判和理由，用户拥有最终决定权。声音制作路线单独使用 `seed-full-track` 或 `post-dub`，下游只读取用户已确认值。
3. `narration-promo` 以正式旁白及其时间戳为第一时间轴；分镜、画面和字幕都服从它。
4. `drama` 以确认剧本、角色身份和导演分镜为第一依据。Seed 默认在视觉资产确认后进入全局配音工作台，先完成角色声音提示词、参考音和完整声音轨；IndexTTS2/Qwen3-TTS 只有在用户选择 `post-dub` 且 `editing-timeline.json` 就绪后才按真实时间窗生成。
...
9. VEO/Grok 仍按原分镜提示词正常生成对白、旁白、音乐、环境声和动作音效；最终是否采用视频原生声音由后期声音路线决定，Seed 路线默认采用 Seed 完整声音轨但不在提示词阶段禁用视频原声。
...
15. 顶部现有“配音”节点升级为“配音字幕”节点；Seed 路线在资产后显示前置“全局配音”视图，后期工作台顺序为 `文稿 -> 项目总监 -> 资产 -> 全局配音（Seed） -> 分镜 -> 分镜图 -> 视频 -> 配音字幕 -> 成片`，逐句后配路线跳过全局配音视图。
16. 产品尚未发布，只实现新流程，不维护旧流程兼容分支。
17. Seed 路线中的豆包语音稿是独立派生文档，不覆盖确认文稿，也不塞入画面导演 Skill。项目总监与声音资产完成后，`jc-doubao-seed-audio` 生成可编辑 `声音导演稿.md`；“生成完整声音轨”只能读取用户当前保存的该稿，不能在按钮内部重新调用 Skill。
...
内容类型与声音制作路线是正交字段。本节 4.1/4.2 描述原有内容流程；用户选择 `seed-full-track` 时，旁白或剧情内容统一先插入以下声音优先链路，完成后才进入各自的画面导演流程：

```text
确认文稿
  -> 项目总监确认内容类型、角色/场景/道具稳定 ID 和导演资料
  -> 资产阶段完成视觉资产；进入全局配音工作台确定角色参考音
  -> 按角色 JSON 生成音色提示词，或选择/绑定已有参考音
  -> 按角色音色提示词生成 Seed 角色参考音并绑定 voiceProfileId
  -> “整段配音安排”：程序确定角色分组、参考音顺序和任务边界
  -> “生成全局声音提示词”：整合角色声音、确认文稿、音乐、环境声和动作音效
  -> “生成豆包语音稿”：jc-doubao-seed-audio 生成可编辑声音导演稿
  -> 用户查看并按需修改声音导演稿
  -> “生成完整声音轨”：Seed Audio 原样消费当前声音导演稿
  -> Faster-Whisper 自动生成对白时间轴和 SRT
  -> 画面导演 Skill 按真实声音时间轴生成分镜
  -> 分镜图和分镜视频
  -> 每条素材 Faster-Whisper SRT
  -> Gemini 读取原视频、完整分镜提示词和素材 SRT，生成 editing-timeline.json
  -> 配音字幕工作台
  -> 成片
```

三份声音上游产物各有唯一职责：`确认文稿.md` 保存内容事实，`整段配音安排.json` 保存确定性任务合同，`声音导演稿.md` 保存 Seed Audio 的最终 `text_prompt`。声音导演稿允许扩写角色音色、表演、音乐、环境声、动作音效、空间/混响和声音事件先后，但不得改写正式台词。画面导演分镜仍是第四层独立合同。
...
项目总监确认 narration-promo
  ...
  -> Veo 或 Grok 生成分镜图和视频
  -> “生成 SRT”：Faster-Whisper 逐条识别素材人声并输出精确时间证据；没有可识别人声时输出合法空 SRT
  ...
  -> 按已确认声音路线采用正式声音轨、视频原生声音或后配音轨，并按需混入环境声和动作音
  -> 按旁白时间轴烧录节拍字幕
  -> 输出成片
```
...
角色声音身份和参考音仍是资产数据，但 Seed 路线的生成/绑定操作不放在资产页。全局配音工作台按角色显示“选择/绑定已有参考音”“生成角色音色提示词”“生成角色参考音”和试听状态；所有有声角色完成后，才开放全局声音提示词和完整声音轨。逐句后配路线仍在 `editing-timeline.json` 就绪后的“配音字幕”工作台调用 IndexTTS2/Qwen3-TTS。
...
5. 视频提示词按确认文稿、声音设计和用户选择的声音路线正常写明对白、旁白、音乐、环境声和动作音效；最终采用哪条声音轨由后期策略决定，不在提示词阶段禁止视频模型生成声音。
...
不得在素材 SRT 就绪前提交剧情片 Gemini 分析。宣传片素材可能包含 VEO/Grok 按提示词生成的对白、旁白或其他声音；“生成 SRT”必须记录实际可识别的人声，确实没有人声时才输出合法空 SRT。Gemini 继续根据原视频、提示词、确认文稿和 SRT 判断目标画面。
...
- VEO/Grok 按原分镜提示词正常生成视频及其原生声音；Seed 路线后期默认采用 Seed 完整声音轨，视频原生声音保留为证据或备用，不默认跑 Spleeter。
...
只生成与当前路线有关的文件。`声音导演稿.md` 是 Seed 路线中确有人工审阅价值的模型输入合同，因此独立落盘；其余运行中状态继续写入对应 JSON，不为每个按钮再造 Markdown。宣传片不创建空的剧情对白资产；剧情片没有公共旁白时不创建旁白方案。`wiki/制作/episode-001.md` 是本集双链索引，必须链接项目总监、确认文稿、声音绑定、声音导演稿、分镜、素材 SRT、剪辑时间轴、配音、音频处理、字幕和成片；各产物也反链该索引。
...
- 剧情片：这里只显示声音路线入口和状态，不生成整集旁白；Seed 的角色声音与全局声音操作进入资产后的“全局配音”工作台。

### 12.4 资产

- 宣传片：角色、场景、道具照常；旁白音色可在文稿/配音或旁白声音行绑定。
- 剧情片：视觉资产页只显示角色的声音资产状态入口，不显示八情绪播放器或虚假的逐句待处理数。
- Seed 路线：资产确认后进入“全局配音”工作台。中栏显示角色列表、角色音色提示词、参考音试听、全局声音导演稿和完整声音轨；右栏按当前角色显示“生成角色音色提示词”“生成角色参考音”，按全局状态显示“整段配音安排”“生成全局声音提示词”“生成豆包语音稿”“生成完整声音轨”“生成分镜提示词”。

### 12.5 分镜与媒体

- 宣传片每镜显示关联旁白 cue、绝对时间和节奏；视频提示词正常包含旁白、画面、环境声和音效要求，后期按声音路线选择最终声音轨。
...
- `烧录配音和字幕` 就是“生成成片”，也是唯一最终动作。中文是默认输出；用户主动生成英语配音后，本次烧录自动使用英语配音和英文字幕。语言由按钮动作推导，不增加成片语言选择器。它读取当前滑块区间和当前显示字幕并裁切原始视频；不另设保存、确认或导出按钮。
...
|---|---|---|---|
| 整段配音安排 | 确定性程序，无模型 | 确认文稿、项目总监角色、`voiceProfileId`/参考音 | `wiki/声音/episode-001/seed-audio/整段配音安排.json` |
| 生成豆包语音稿 | `jc-doubao-seed-audio` | 确认文稿、整段安排、声音资产及声音要求 | 可编辑 `wiki/声音/episode-001/seed-audio/声音导演稿.md` |
| 生成完整声音轨 | Seed Audio + FFmpeg + Faster-Whisper | 用户当前保存的声音导演稿、安排中的真实参考音 | 完整声音轨、生成记录、声音时间轴和 SRT |
...
4. 中文字幕合法；
5. 用户已生成英语配音时，英文字幕和英语配音都与当前中文字幕版本一致；默认中文输出不要求生成英文；
...
| 确认文稿或 Seed 角色声音绑定 | 整段安排、声音导演稿、完整声音轨、声音时间轴、分镜及全部下游 | 历史文件和未受影响的视觉资产 |
| 声音导演稿 | 完整声音轨、声音时间轴、分镜及全部下游 | 确认文稿、项目总监、声音资产、整段安排 |
...
### 14.1.1 豆包整段声音轨（旁白或剧情）

1. 输入并确认文稿，进入项目总监并确认内容类型、资产白名单和导演资料。
3. 生成视觉资产并确认。
4. 进入全局配音工作台，逐角色选择/绑定参考音；没有参考音时根据角色 JSON 生成音色提示词，再生成角色参考音并绑定产品 `voiceProfileId`。
5. 点击“整段配音安排”，检查角色分组、参考音顺序和任务边界。
6. 点击“生成全局声音提示词”，再点击“生成豆包语音稿”；在中栏检查并编辑声音导演稿。
7. 点击“生成完整声音轨”；APP 原样提交当前语音稿并自动运行 Faster-Whisper，生成真实对白时间轴和 SRT。
8. 点击“生成分镜提示词”，画面导演按真实声音时间轴生成分镜；随后生成分镜图和视频。
9. 每条素材生成 SRT，Gemini 再读取原视频、完整分镜提示词和素材 SRT，输出真实剪辑点。
10. 进入后期配音字幕工作台校准，最后烧录声音和字幕输出成片。
...
4. Seed 路线必须先生成可编辑 `声音导演稿.md`；生成完整声音轨时发送的 `text_prompt` 与用户当前保存正文一致，不能重新调用 Skill 覆盖修改。
5. 宣传片使用设计声音和克隆音色包两种方式都能生成正式旁白。
6. 宣传片旁白生成后，Faster-Whisper 输出合法 JSON/SRT，正式文字与确认文稿一致。
7. 宣传片分镜总时长等于正式旁白时长，每镜均回链旁白 cue。
8. VEO/Grok 按宣传片提示词正常生成原生声音；最终按已确认声音路线采用 Seed 完整轨或视频原生轨，不能通过提示词硬禁用声音。
9. 剧情片资产阶段只显示有台词角色的真实音色绑定，不预创建逐句记录或虚假待处理状态。
10. `editing-timeline.json` 就绪后 IndexTTS2 才生成逐句中文配音；单句失败或时间窗变化只重试受影响句，其他成功对白不丢失。
11. “生成 SRT”按钮为每条采用视频输出包含实际识别文字和精确人声时间的 SRT；不使用分镜台词替换识别结果。
12. Gemini 请求只有在素材 SRT 就绪后才能提交，并同时包含原视频、确认剧本、完整分镜视频提示词和这份 SRT。
13. Gemini 能通过 SRT 台词与确认剧本直接确定 `speakerId`，通过画面识别无对白戏剧动作，并输出回链 `promptSegmentId` 的 `editing-timeline.json`；无法可靠确定剪辑点时停止自动裁切。
14. Sherpa-ONNX + Spleeter 生成可用的人声与背景声文件，最终丢弃原人声并保留环境声。
15. FFmpeg 的每个裁切区间都能追溯到 Gemini 原始剪辑点或用户人工修订点；代码不存在截前 N 秒或自行推算区间的旁路。
