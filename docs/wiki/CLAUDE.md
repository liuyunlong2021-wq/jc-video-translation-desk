...
当前代码已完成共享合同、项目总监、Seed 完整声音轨、声音绑定、素材 SRT、Gemini 剪辑时间轴、逐句中英文配音、音频处理和字幕成片链路。真实完整项目的付费模型端到端验收仍待执行；完整状态以 [[hot|当前状态与风险]] 为准：

```text
创建或打开项目
  -> 输入需求、导入 Markdown，或通过视频反推生成文稿草稿
  -> 生成、编辑并确认正式文稿
  -> 项目总监判断旁白宣传片或剧情片的内容类型
  -> 默认进入豆包整段声音轨；用户明确选择后才进入逐句后配路线
  -> 为角色、场景、道具生成资产设计 JSON
  -> 可选搜索/上传参考图，生成并确认资产图
  -> Seed 路线进入全局配音工作台：逐角色选择/绑定参考音、生成角色音色提示词、生成角色参考音
  -> 生成全局声音提示词、可编辑豆包语音稿和完整声音轨
  -> Faster-Whisper 生成真实声音时间轴后才生成分镜提示词
...
现行产品主链见 [[架构/项目Wiki与资产主导工作流]]，逐镜成片合同见 [[架构/逐镜智能剪辑与声音成片]]，视频反推输入见 [[开发/视频反推剧本输入路线]]，Seed 实现见 [[开发/Seed Audio声音优先路线]]，哪些能力已经验证、哪些仍待验收，以 [[hot|当前状态与风险]] 为准。
...
Electron 主进程
  ...
  v
用户选择的可读项目目录（projectId 通过应用注册表解析）
```
...
每个新创作项目位于用户选择或新建的空文件夹，该文件夹本身就是项目根，文件夹名就是初始项目显示名。Electron `userData/media-projects.json` 只保存稳定 `projectId -> rootPath` 注册关系；API Key、数据库、模型缓存和全局声音库仍留在应用数据目录。项目根内容如下：

```text
project.json          稳定 projectId、项目名称、剧集清单和结构版本
shared-state.json     项目级共享状态
episodes/             按 episodeId 隔离的状态和媒体
run.json              云端媒体任务账本
.raw/                 用户原始提交和导入资料快照
...
episodes/<episodeId>/picture-master.mp4  裁切拼接后的画面母版
episodes/<episodeId>/final.mp4           最终成片
```
...
- 所有项目文件只保存项目相对路径；机器绝对根目录只存在应用级注册表。项目移动后通过“打开已有项目目录”重新注册，不改项目内 ID、Wiki 或媒体路径。
- 资产类型固定为角色、场景、道具；商品也属于道具。资产设计 JSON 是生图正式合同。`search | upload | generated` 记录来源，`activeVersionId` 独立决定当前正式使用图，任意来源都可直接采用。
...
| 制作路线 | 默认 `seed-full-track`；用户明确选择后才使用 `post-dub` |
...
| 配音 | Seed 全局配音工作台按角色生成/绑定参考音、生成可编辑声音导演稿并生成整段声音轨；Qwen3-TTS/IndexTTS2 保持现有逐句后配路线 |
| 视频原生声音 | VEO/Grok 继续按原提示词正常生成；最终采用由声音路线决定，不在提示词阶段禁用 |
...
- [[架构/Gemini3.6Flash真实能力与受控工作流|Gemini 3.6 Flash 真实能力与受控工作流]]
- [[架构/Veo3.1能力边界与镜头节奏|Veo 3.1 能力边界与镜头节奏]]
- [[开发/三栏工作区与项目级媒体任务|三栏工作区与项目级媒体任务]]
- [[开发/视频反推剧本输入路线|视频反推剧本输入路线]]
