...
创建或打开项目
  ...
  -> 可选搜索/上传参考图，生成并确认资产图
  -> 导演 Skill 生成逐镜分镜并绑定稳定资产 ID
  -> 逐镜生成分镜图和 4/6/8 秒视频
  -> Gemini 逐镜分析动作、说话窗口和裁切建议
  -> ffprobe 校验，FFmpeg 生成画面母版
  -> 按声音策略完成配音、混音、字幕和最终成片
```
...
| 单镜头视频 | `veo-3.1-generate-preview`；支持 `9:16`、`16:9` 和 4/6/8 秒 |
| 逐镜分析 | `gemini-3.6-flash` 直读每条原始视频与声音，不抽帧 |
| 媒体执行 | `ffprobe` 校验，FFmpeg 确定性裁切、拼接、混音和字幕 |
| 配音 | 云端声音能力；本地模式仅面向具备独立运行时和模型的 Apple Silicon macOS |

模型只负责生成建议或媒体，不能直接决定最终剪辑。`editing-timeline.json` 是剪辑合同，`ffprobe` 负责校验真实时长，FFmpeg 是最终执行器。更完整的时长规则见 [[架构/Veo3.1能力边界与镜头节奏]]。
